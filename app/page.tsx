...
function Login({ onError }: { onError: (message: string) => void }) {
  ...
  return <main className="auth-shell">
    ...
    <section className="auth-panel"><div className="auth-form-wrap"><div className="mobile-brand"><img className="brand-mark brand-logo" src="/my%20desk%20logo.png" alt="My Desk logo" /><span>My Desk</span></div><p className="eyebrow">{forgotPasswordOpen ? "Password reset" : registering ? "New account" : "Welcome back"}</p><h2>{forgotPasswordOpen ? "Reset your password" : registering ? "Create your account" : "Sign in to My Desk"}</h2><p className="muted">{forgotPasswordOpen ? "Enter your account email and we will send a password reset link." : registering ? "Create your account and verify your email to get started." : "Enter your details to continue."}</p>
...
function PermitNotificationCenter({ isAdmin, count, notifications, onAdminOpen, onViewPermit }: {
  isAdmin: boolean;
  count: number;
  ...
  return <div className="notification-center" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>
    <button type="button" className="notification-button" aria-label={`${count} ${isAdmin ? "pending Permit Slip" : "approved or disapproved Permit Slip"} notifications`} title={`${count} ${isAdmin ? "pending Permit Slip" : "approved or disapproved Permit Slip"} notifications`} aria-haspopup={isAdmin ? undefined : "dialog"} aria-expanded={isAdmin ? undefined : open} onClick={() => { if (isAdmin) { setOpen(false); onAdminOpen(); } else setOpen((current) => !current); }}>
    ...
    {!isAdmin && open && <div className="notification-dropdown" role="dialog" aria-label="Permit Slip notifications">
      <div className="notification-dropdown-heading"><strong>Permit Slip updates</strong><button type="button" onClick={() => setOpen(false)} aria-label="Close notifications">×</button></div>
      {notifications.length === 0 ? <p className="notification-empty">No approved or disapproved Permit Slips yet.</p> : <ul>{notifications.map((notification) => <li key={`${notification.permit.id}-${notification.permitNo}`}><button type="button" className="notification-item" onClick={() => { setOpen(false); onViewPermit(notification); }}>
        <span className={`notification-status notification-status-${notification.status.toLowerCase()}`}>{notification.status}</span>
        ...
        <span className="notification-view-label">View preview →</span>
      </button></li>)}</ul>}
    </div>}
  </div>;
}
...
export default function Home() {
  ...
  const [pendingPermitNotifications, setPendingPermitNotifications] = useState(0);
  ...
  useEffect(() => {
    if (!user || !db || !isPermitAdmin(user.email)) {
      setPendingPermitNotifications(0);
      return;
    }
    const firestore = db;
    return onSnapshot(collection(firestore, "permits"), (snapshot) => {
      const pendingCount = snapshot.docs.reduce((total, item) => {
        const data = item.data() as Record<string, unknown>;
        const names = Array.isArray(data.names) ? data.names as string[] : typeof data.name === "string" ? [data.name] : [];
        const permit = { ...data, id: item.id, names } as Permit;
        return total + names.filter((_, index) => (permit.personStatuses?.[permitDecisionKey(permit, index)]?.status ?? "Processing") === "Processing").length;
      }, 0);
      setPendingPermitNotifications(pendingCount);
